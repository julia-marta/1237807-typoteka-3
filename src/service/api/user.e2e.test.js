...
const createAPI = async () => {
  ...
  await initDB(mockDB, {categories: [], articles: [], users: [], comments: []});
...
describe(`API creates first user only with admin role`, () => {

  const firstUser = JSON.parse(JSON.stringify(mockUser));
  const secondUser = JSON.parse(JSON.stringify(mockUser));
  secondUser.email = `user2@ya.ru`;
  ...
  beforeAll(async () => {
    app = await createAPI();
    firstResponse = await request(app).post(`/user`).send(firstUser);
    secondResponse = await request(app).post(`/user`).send(secondUser);
  });

  test(`Returns first user with admin role equals true`, () => expect(firstResponse.body.admin).toEqual(true));
  test(`Returns second user with admin role equals false`, () => expect(secondResponse.body.admin).toEqual(false));
});
